...
CJ, 2026-09-22: 「i want music for different section can you find 1980s 1990s 2000s 2010s music and make a music player system, i can switch when i am in that decade」. CJ, 2026-09-23: 「close it , iwant spotify embed with playlist」 (closing the private-folder and per-track directions that came between). One Spotify playlist embed sits inside the control bottom-left; the scroll year picks the decade's playlist, the four buttons switch it by hand, and the synthesised loops are only the offline fallback.

| Decade | Scroll years | Spotify playlist (editorial) | Local fallback loop |
|---|---|---|---|
| 1980s | up to 1989 | 80 年代華語經典 `spotify:playlist:37i9dQZF1DX1sUunABTZ4W` | `asset/music/1980s.mp3` Neon Arcade, city pop |
| 1990s | 1990–1999 | 90年代華語精選 `spotify:playlist:37i9dQZF1DX7zXqDWRZPD1` | `asset/music/1990s.mp3` Letters Home, piano ballad |
| 2000s | 2000–2009 | 2000年代華語金曲 `spotify:playlist:37i9dQZF1DWVUmQhB7PvFH` | `asset/music/2000s.mp3` Night Market Slow Jam, R&B |
| 2010s | 2010 onward, including the tower chapter | 2010年代華語最流行 `spotify:playlist:37i9dQZF1DXe3opFF4aPDr` | `asset/music/2010s.mp3` Skyline Pulse, EDM pop |

- **Files.** `music.js` (the player, loaded last), the control markup in `index.html` (`#music`, with `#musEmbed` that the Spotify API replaces by its iframe) and its styles in `style.css` (`.hud-music`), the four MP3s (112 kbps, 4.1 MB total) and `asset/music/CREDITS.md`. The loops are synthesised in the repo by `asset/music/gen/make-tracks.js` (seeded, no samples, CC0); rebuild with `node asset/music/gen/make-tracks.js --mp3` (needs ffmpeg). `app.js`, `data.js` and the scenes are untouched: the player polls `window.__fog.year` once per frame.
- **Swap a playlist.** Edit the `SPOTIFY` table at the top of `music.js`: `uri` is `spotify:playlist:<id>` from the playlist's share link, `name` and `by` are what the control shows. Any public playlist works; a private one shows as unavailable.
- **The network exception.** This is the page's one network dependency, opt-in by CJ's decision above: `music.js` loads `https://open.spotify.com/embed/iframe-api/v1` and the embed iframe. Everything else still runs from a double-clicked `index.html`. Verified 2026-09-23 that the embed loads from `file://` in Chrome (headless and headed), no http server needed. Without a Spotify login the embed plays 30-second previews; logged-in listeners get full tracks.
- **Gesture and fallback.** Browsers block sound until a click, tap or key press. The first click on the page calls `play()` on the embed (or its own play button works); the toggle then reads Play / Pause and the `m` key toggles it. If the API script fails or no player is ready within 8 s (offline), the control reads "offline: local loops", the embed hides, and the loops play through the old two-`<audio>` crossfade with the same gesture gate ("Click for sound", then Sound on / off, mute stored in `localStorage`). `index.html?nospotify=1` rehearses that path. A manual decade choice holds until the scroll enters a decade other than the chosen one, then the scroll takes over again. On a phone (≤ 480 px) the control keeps the embed and the buttons and drops the text lines, above the year.
- **Real songs.** `research/music/README.md` lists five recognisable hits per decade with rights holders and what a sync plus master licence would take, from before the Spotify decision. Still a decision for CJ if he ever wants a specific recording in the page itself; nothing copyrighted is in the repo.
- **Testing.** `window.__music` exposes `mode` (`pending` / `spotify` / `local`), `decade`, `playlist`, `loadedUri`, `track`, `playing`, `paused`, `gestured`, `manual`, `embed`, `setDecade(key)`, `toggle()`. Mute the page right after `open` (`agent-browser eval "document.querySelectorAll('audio').forEach(a=>a.muted=true)"`): new headless Chrome plays audio through the speakers, and the Spotify iframe cannot be muted from outside, so do not click the page in a session you leave open. Close every session at the end of a check. Do not use `agent-browser press` or `reload` on this page: both wedged the session daemon repeatedly (os error 35); dispatch a `KeyboardEvent` from `eval` and re-`open` the URL instead. Measured 2026-09-23: 100.4 / 100.3 / 100.3 fps headed at 0.2 / 0.44 / 0.78 with the embed loaded, against 100.4 / 100.4 / 100.3 on main.
